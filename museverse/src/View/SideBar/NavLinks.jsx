...
const links = [
  ...
  { name: "Favorites", to: "/artists", icon: BiSolidPlaylist },
...
export default function NavLinks({ handleClick, userPlaylist = [] }) {
    ...
    <div className={`${expanded ? "" : ""}`}>
      ...
      <div className="overflow-auto">
        {links.map((item) => (
          ...
          >
            <item.icon
              className={`w-10 h-7 ${expanded ? "mr-2" : "ml-[3px]"}`}
            />
            <span
              className={`overflow-hidden transition-all ${
                expanded ? "w-56 ml-3" : "visibility: hidden"
              }`}
            >
              {item.name}
            </span>
      ...
      <h2
        className={`overflow-hidden transition-all text-gray-500 font-semibold ${
          expanded ? "" : "visibility: hidden"
        }`}
      >
        My Playlist
